...
- `internal/ui/root.go` — `RootModel`, tab dispatch, help overlay, command palette (`:save`, `:view`, `:del`, `:tab`, `:help`, `:q`), view-recall on `1`–`9`. Tracks the latest `SyncStatusMsg` and renders it as the right-side status-bar indicator (`prefetching N…` / `synced 5s ago` / `offline: <err>`). Broadcasts the data messages listed below to all tabs.
...
- **SQLite cache + Syncer + Query DSL + Saved views** — the v2 backbone.
